...
from . import tests, TESTS_LIST, HYP_NAMES, ORDINALS
...
def find_treatments(mother_dir, batch_num, task):

    test_dir = [path for path in mother_dir.glob(f"**/*{task}*") if path.is_dir()][0]

    if not test_dir:
        raise Exception(f"No test directory with {task} found.")

    batch_ord = f"{ORDINALS[int(batch_num)-1]} Batch"
    treatment_dirs = [path for path in test_dir.iterdir() if path.is_dir() and batch_ord in path.name]
    substance_dict = {}
    parameters_required_dict = {}
    for dir_path in treatment_dirs:
...
def get_current_params_count(project_dir='a path', batch_num=1, task='a task', treatment_index='A'):
    ...
    task_index = TESTS_LIST.index(task)
...
def autoanalyzer(PROJECT_DIR, BATCHNUM, TASK, PROGRESS_BAR, OVERWRITE = False, skip_list = {}):
    ...
    TREATMENTS, PARAMS_REQUIRED = find_treatments(PROJECT_DIR, BATCHNUM, TASK)
    ...
    # CHECK POINT 1
    logger.info(f'Loaded {len(TESTS)} tests: {[TESTS_LIST[int(t)] for t in list(TESTS.keys())]}')
    ...
    # =========================================================
        ...
        test_num = TESTS_LIST.index(TASK)
